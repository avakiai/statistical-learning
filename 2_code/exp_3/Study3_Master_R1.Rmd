...
source(file.path(code_path,'../../', '2_code', 'summarySE.R'))
source(file.path(code_path,'../../', '2_code','R_rainclouds.R'))
...
# Following Reviewer Comments, we added stream position to the log files: 
# rt_data_orig <- read_csv(file.path(data_path,'RT_response_table_R1.csv'))
...
# Edited: Featuring the stream position info:
# Use this to examine the chunk "Reviewer suggested model for stream position confound"
# rt_data <- read_csv(file.path(data_path,'exp_3_rt_data_R1.csv')) %>%
#       mutate(subject = as.factor(subject),
#            block = as.factor(block),
#            target =  factor(target, levels = c("nu","ga","di","ro","ki","se","mi","po","la","za","be","tu")),
#            tgt_pos = as.factor(tgt_pos),
#            tgt_word = as.factor(tgt_word))
...
rt_data_cleaned.1 <- rt_data_clean %>% 
                     dplyr::select(-c("subject","sequence_file")) %>%
...
# write.csv(rt_data,file.path(data_path,'exp_3_rt_data_R1.csv'), row.names = FALSE)
```

# 1 - PLOTS
## RT is predicted by triplet (target) position
### Fig. 1a. Plot RT ~ Pos
...
ggplot() +
 ...
 # scale_colour_brewer(palette = "Paired") +
  ...
  ylab('Median RT (s)') + xlab('Triplet position') +
  theme_classic() +
...
### Plot RT ~ Syllable
```{r message=FALSE, warning=FALSE}
rt_data_sum.ex <- rt_data %>% # target position x subject
    summarySE(measurevar = "rt", groupvars = c("tgt_pos","subject","target"),na.rm=TRUE) %>%
  mutate(target = factor(target, levels = c("nu","ga","di","ro","ki","se","mi","po","la","za","be","tu")))
rt_data_sum.ex2 <- rt_data %>% # target position x subject
    summarySE(measurevar = "rt", groupvars = c("tgt_pos","target"),na.rm=TRUE) %>%
  mutate(target = factor(target, levels = c("nu","ga","di","ro","ki","se","mi","po","la","za","be","tu")))

ggplot(rt_data_sum.ex) +
  geom_point(mapping = aes(x = target, y = rt_median, color = tgt_pos)) +
    geom_smooth(data = rt_data_sum.ex2, mapping = aes(x =target, y = rt_median, group=1), ci=TRUE,size=1, colour="BLACK") +
  #geom_errorbar(rt_data_sum.ex2, mapping = aes(x = target, y = rt_median, ymin = rt_median-ci, ymax = rt_median+ci), colour = "BLACK", width = 0.1, size = 0.8) +
  #geom_line(data = rt_data_sum.ex2, mapping = aes(x = target, y = rt_median, group = 1), color = "Black", size = .9) +
  scale_colour_brewer(palette = "Dark2") +
  labs(colour= "Triplet position") + ylab('Median RT (ms)') + xlab('Target syllables') +
  theme_classic() +
  theme(text = element_text(family = "LM Roman 10", face="bold", size = 22), legend.position = "top") +
  ggsave(file.path(fig_path,'SM_SyllVar3.png'), width = w, height = h)
```

### Plot for each Subject: RT ~ Pos x Block - See Chunk 45
...
# LOAD THE MODEL ALREADY COMPUTED
...
## Evaluating the effect of blocks
...
b12.pos <- emmeans(block12.mod, specs = pairwise ~ tgt_pos, adjust = "tukey", transform = "response")
b12.dpos <- as.data.frame(b12.pos$contrasts)$estimate / sigmaHat(block12.mod)

b12.block <- emmeans(block12.mod, specs = pairwise ~ block, adjust = "tukey", transform = "response")
b12.d <- as.data.frame(b12.block$contrasts)$estimate / sigmaHat(block12.mod)
...
### Block x Pair Differences: Contra Himbergen et al. 
```{r}
...
emmip(pairings.glm, ~ pairs, CIs=TRUE, type = "response") + 
...
emmip(pairings.glm, block ~ pairs, CIs=TRUE, type = "response") + 
  scale_x_discrete(name = "Mean RT diff. between positions", labels=c("1-2","2-3","1-3")) +
  scale_y_continuous(name="Est. marginal means (ms)") +
...
### Edit: Reviewer suggested model for stream position
"An additional step that the authors could carry out to confirm that stream position does not account for the effect is to run a mixed effects model for their analysis of reaction times that include factors of triplet position, as well as stream position, and possibly block (if desired)."
```{r}
# Suggested model
reviewer2.mod <- glmer(rt_secs ~ 1 + tgt_pos + stream_pos + (1 | subject), data = rt_data, 
                        family = Gamma(link = "log"),
                       control = glmerControl(optimizer="bobyqa", optCtrl=list(maxfun=2e5)))
summary(reviewer2.mod)
Anova(reviewer2.mod)


nrow(rt_data)
length(getME(reviewer2.mod,"theta"))
length(fixef(reviewer2.mod))
# Check Singularity
...
tt <- getME(reviewer2.mod,"theta")
ll <- getME(reviewer2.mod,"lower")
min(tt[ll==0])
# Not a problem!

# Check gradient calculations
# Compare absolute and scaled gradient; compare gradient and Hessian with numDeriv equivalents.
derivs1 <- reviewer2.mod@optinfo$derivs
sc_grad1 <- with(derivs1,solve(Hessian,gradient))
max(abs(sc_grad1))

# This is smaller, although still larger than the tolerance we typically set (0.001).
ss <- getME(reviewer2.mod,c("theta","fixef"))
reviewer2.mod2 <- update(reviewer2.mod,start=ss,control=glmerControl(optimizer="bobyqa",
                            optCtrl=list(maxfun=2e5)))

reviewer2.mod3 <- update(reviewer2.mod3,start=ss,control=glmerControl(optimizer="bobyqa",
                            optCtrl=list(maxfun=2e5)))
derivs1 <- reviewer2.mod3@optinfo$derivs
sc_grad1 <- with(derivs1,solve(Hessian,gradient))
max(abs(sc_grad1))
reviewer2.mod4 <- update(reviewer2.mod3,start=ss,control=glmerControl(optimizer="Nelder_Mead",
                            optCtrl=list(maxfun=2e5)))


summary(reviewer2.mod3)
Anova(reviewer2.mod3)
(reviewer2.emm <- emmeans(reviewer2.mod3, spec = ~ tgt_pos | stream_pos, adjust = "Tukey", transform = "response"))


# --- Linear model version?
reviewer2.lm <- lm(rt_secs ~ 1 + tgt_pos + stream_pos, data = rt_data)
summary(reviewer2.lm)
Anova(reviewer2.lm)

(reviewer2.emmlm <- emmeans(reviewer2.lm, spec = ~ tgt_pos | stream_pos, adjust = "Tukey", transform = "response"))


```
Model fails to converge when we attempt to predict RT_secs using target position, stream position, and block, with subject as random intercept factor. Also when removing block. 
When predicted using only stream position, same issue. When predicted using only stream position as factor, model never seems to resolve. 
Using RT (ms) and target position and stream position (but not their interaction), model is unidentifiable. 

#### Fig. S2e-f: Stream Position
```{r}
# First, a plot to show that the targets appeared in all stream positions
ggplot(rt_data, aes(target,stream_pos)) +
  geom_jitter(width = 0.01) +
  scale_y_continuous(name = "Stream position") +
  scale_x_discrete(name = "Target syllable") + 
  theme_classic() +
  theme(text = element_text(family = "LM Roman 10", face="bold",size = 20),
        legend.position = "top") +
  ggsave(file.path(fig_path,'R1_Stream_positions.png'),width=7,height=6)

stream.sum <- summarySE(rt_data,measurevar = "rt_secs",groupvars = c("tgt_pos","stream_pos"),na.rm = TRUE)
stream.sum.block <- summarySE(rt_data,measurevar = "rt_secs",groupvars = c("tgt_pos","stream_pos","block"),na.rm = TRUE)

ggplot(stream.sum, aes(stream_pos,rt_secs_median,color=tgt_pos)) + 
  geom_point() + 
  scale_color_brewer(palette="Dark2", name = "Position") + 
  ylab('Median RT (s)') + xlab('Stream position') +
  theme_classic() +
  theme(text = element_text(family = "LM Roman 10", face="bold",size=22), legend.position = "top") +
  ggsave(file.path(fig_path,'R1_S6_Stream_position.png'), width = w, height = 6)

ggplot(stream.sum.block, aes(stream_pos,rt_secs_mean,color=tgt_pos)) + 
  geom_point() + 
  ...
  ylab('Median RT (s)') + xlab('Stream position') +
  theme_classic() +
  theme(text = element_text(family = "LM Roman 10", face="bold",size=18), legend.position = "top") +
  ggsave(file.path(fig_path,'R1_S6_Stream_position_block.png'), width = w, height = h)
...
```{r}
rt.mod.lm <- lm(rt_secs ~ 1 + tgt_pos * subject, data = rt_data)
  summary(rt.mod.lm)
  plot(residuals(rt.mod.lm))
  qqnorm(resid(rt.mod.lm))
  Anova(rt.mod.lm,type = 2)
  
  (subs.pairs <- emmeans(rt.mod.lm, specs = pairwise ~ tgt_pos | subject, adjust = "Bonferroni", transform = "response"))

  subj.contrasts <- as.data.frame(subs.pairs$contrasts) %>%
                mutate(effect = case_when(p.value < 0.05 ~ 1))
  
 
  n_subjs_effect_1to3 <- length(
    unique(
      subj.contrasts$subject[
        intersect(which(subj.contrasts$effect==1), 
        union(which(subj.contrasts$contrast=="1 - 3"),which(subj.contrasts$contrast=="1 - 2"))  
                  )  
                  ]))
...
rt_data_sum4.0 <- rt_data %>% # target position x subject
    summarySE(measurevar = "rt", groupvars = c("tgt_pos","subject"),na.rm=TRUE) %>%
    mutate(effect = as.factor(case_when(subject %in% subjs_effect_1to3 ~ "present",
                                        TRUE ~ "not present") )) 

ggplot() + 
  geom_point(rt_data_sum4.0, mapping=aes(x=tgt_pos,y=rt_median), color="BLACK") +
  geom_line(rt_data_sum4.0, mapping=aes(x=tgt_pos,y=rt_median,group=1, color=effect)) +
  geom_errorbar(rt_data_sum4.0, mapping=aes(x=tgt_pos,y=rt_median,ymin=rt_median-ci,ymax=rt_median+ci, color=effect),width=.3) +
  ylab('Median Response Time (ms) [bars = CI]') + xlab('Triplet Postion') + labs(color="RT effect (1st-3rd)") +
  scale_color_manual(values = c("black","red")) + 
  facet_wrap(. ~ subject, ncol=6) +
  theme_classic() +
  theme(text = element_text(family = "LM Roman 10", face="bold", size = 18),legend.position = "bottom") +
  ggsave(file.path(fig_path,'Supplementary_SubjectsRT.png'),width=17,height=10)
```
...
#  scale_color_manual(values = c("#ABABAB", "#757575", "#454545")) + 
  ...
  guides(fill = FALSE) + labs(color = "Triplet position") +
  theme_classic() +
  theme(text = element_text(family = "LM Roman 10", face="bold",size = 22),
        legend.position = "top") +
