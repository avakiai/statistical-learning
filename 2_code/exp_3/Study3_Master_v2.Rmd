...
#  scale_y_continuous(limits=c(380,580)) +
  ...
  theme_classic() +
  theme(text = element_text(family = "LM Roman 10", face="bold",size=15)) +
  ggsave(file.path(res_path,'figures','fig1b_rt_pos_grey.png'), width = w, height = h)
...
png(file = file.path(res_path,'figures','block_interaction_withlegend_inset.png'), width = w, height = h, res = 300, units = "in")
emmip(rt.mod.full.int.fct, tgt_pos ~ block, CIs = TRUE, xlab = "Block",type = "response") +
 theme_classic() +
  ...
  theme(text = element_text(family = "LM Roman 10", face="bold", size = 15),legend.position = "top")
dev.off()
...
shapiro.test(residuals(pairings.aov)) ## oops, residuals not really normal... should I do a glm? or kruskall-wallis?
(pairings.em <- emmeans(pairings.aov, spec = pairwise ~ pairs, adjust = "Tukey", transform = "response"))

png(file = file.path(res_path,'figures','pos_interaction.png'), width = w, height = h, res = 300, units = "in")
emmip(pairings.aov, ~ pairs, CIs=TRUE, type = "response") + 
  scale_x_discrete(name = "Mean RT differences between ordinal positions", labels=c("1-2","2-3","1-3")) +
  scale_y_continuous(name="Estimated marginal means (ms)") +
  theme_classic() +
  theme(text = element_text(family = "LM Roman 10", face="bold", size = 15)) 
dev.off()

png(file = file.path(res_path,'figures','posblock_interaction.png'), width = w, height = h, res = 300, units = "in")
...
ggplot(wordrec_data_wrd, mapping = aes(x=word,y=n_chosen, fill = word)) +
  ...
  scale_y_continuous(name="Number correct (out of 4)",limits=c(0,4.5)) + 
  # theme
...
ggplot(data = plot.corr, aes(d.med, prop.word, color = d.pos)) + 
  ...
  xlab("Scaled median RT differnce") + 
...
annotate(geom = "text", x = 1.3, y = 0.35, label = paste0("\u03C1 = ",round(corrtest.1.3$estimate, digits = 2),", p = ",round(corrtest.1.3$p.value, digits = 2)), 
         size = 4, color = RColorBrewer::brewer.pal(3,"Dark2")[3]) + #"#3C5E4D") +   
...
ggplot(data = corr.batterink2017, aes(delta, prop_word_chosen)) + 
 ...
 # scale_color_manual(values = c("#D44739", "#FAC869", "#3C5E4D"),
#                     name = NULL, labels = c("1st - 2nd", "2nd - 3rd", "1st - 3rd")) +
  xlab("RT score (as per Batterink & Paller 2017)") + 
  ylab("% correct (out of 16)") +
