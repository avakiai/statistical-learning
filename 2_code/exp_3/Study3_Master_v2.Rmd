...
source(file.path(code_path, "/R_rainclouds.R"))
...
```
Remove data sets with technical/programming errors... N = 33
```{r eval=FALSE, include=FALSE}
...
```
Later, keep only these 33 participants when you correlate with word recognition or tempo task. 
```{r eval=FALSE, include=FALSE}
...
dev.copy(pdf,file.path(file.path(res_path,'/figures'),'exp3fig1a.pdf'), width = 6, height = 4)
dev.off()
```
...
dev.copy(pdf,file.path(file.path(res_path,'/figures/'),'exp3fig1b.pdf'), width = 6, height = 4)
dev.off()
```
...
dev.copy(pdf,file.path(file.path(res_path,'/figures/'),'exp3fig2.pdf'))
dev.off()
...
# 
# # png(file.path(file.path(res_path,'/figures/'), '/target_glmm.png'), width=480,height=480,bg = "white")
...
plot(emm.pos, comparisons = TRUE,
  ...
  theme_bw()
#dev.copy(pdf,file.path(file.path(res_path,'/figures/'),'exp3fig1c.pdf'), width = 6, height = 4)
dev.off()
...
emmip(rt.mod.less.int, ~ tgt_pos, xlab = "Target Position", CIs = TRUE) +
  theme_bw()
#dev.copy(pdf,file.path(file.path(res_path,'/figures/'),'exp3figS1.pdf'), width = 6, height = 4)
dev.off()
...
plot(emm.pos.block, comparisons = TRUE,
     ...
     ylab = "ordinal position of target syllable") +
   theme_bw() 
dev.copy(pdf,file.path(file.path(res_path,'/figures/'),'exp3figS2a.pdf'), width = 6, height = 4)
dev.off()
...
dev.copy(pdf,file.path(file.path(res_path,'/figures/'),'exp3figS2b.pdf'), width = 6, height = 4)
dev.off()
...
dev.copy(pdf,file.path(res_path,'/figures/','exp3fig3brolling.pdf'), width = 12, height = 10)
dev.off()
