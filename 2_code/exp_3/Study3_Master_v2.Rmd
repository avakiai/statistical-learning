...
```{r}
check.sum <- rt_data %>% summarySE(.,measurevar = "rt",groupvars = c("target_num"),na.rm = TRUE)

ggplot(check.sum) +
  geom_point(aes(target_num,rt_mean), size = 2) +
  geom_line(aes(target_num,rt_mean, group = 1)) + 
  geom_smooth(aes(target_num,rt_mean, group = 1), color = "red") + 
...
rt_data <- rt_data %>% mutate(trial = as.numeric(trial),target_num = as.numeric(target_num))
...
# 
# # less 2 
# check.sum3.glm <- lm(rt_secs ~ block*tgt_pos, data = rt_data)
# summary(check.sum3.glm)
# plot(check.sum3.glm)
# Anova(check.sum3.glm)
# 
# 
...
# 
# # Model full
# (emmeans(check.sum.glm, specs = pairwise ~ target_num|block))
# 
# 
