...
#  scale_fill_manual(values = c("#ABABAB", "#757575", "#454545")) + 
#  scale_y_continuous(limits=c(380,580)) +
  ylab('Median RT (ms)') + xlab('Ordinal postion') +
  guides(fill = FALSE, color = FALSE) + 
  theme_classic() +
  theme(text = element_text(family = "LM Roman 10", face="bold",size=22)) +
  ggsave(file.path(fig_path,'fig1b_rt_pos_grey.png'), width = w, height = h)
```


## Plot RT ~ Pos x Block x Subject
```{r message=FALSE, warning=FALSE}
...
# AK: Do the pairings change magnitude between blocks?
...
# Lucia: Does the difference between 1-2 and 2-3 change over blocks? 
# I'll do a regular ANOVA here because the outcome variable is normally distributed (means)
pairings.aov <- aov(mean_rt_delta ~ pairs*block,block.matters)
summary(pairings.aov)
TukeyHSD(pairings.aov, which = "pairs")
lsr::cohensD(block.matters$mean_rt_delta[block.matters$pairs=="c1.2"],block.matters$mean_rt_delta[block.matters$pairs=="c2.3"])
## check variance
plot(pairings.aov,1)
car::leveneTest(mean_rt_delta ~ pairs*block,block.matters) # variance across groups is homogenous
plot(pairings.aov,2)
# check normality
ks.test(residuals(pairings.aov),rnorm(length(residuals(pairings.aov))))
shapiro.test(residuals(pairings.aov)) ## oops, residuals not really normal... go back and do glmer
(pairings.em <- emmeans(pairings.aov, spec = pairwise ~ pairs, adjust = "Tukey", transform = "response"))

descdist(block.matters$mean_rt_delta[!is.na(block.matters$mean_rt_delta)])
# ---> the dist is approximately normal, so i'll use a linear model
...
block.aov<- aov(rt_mean ~ block, block.minim)
summary(block.aov)

t.test(block.minim$rt_mean[block.minim$block==1],block.minim$rt_mean[block.minim$block==8])

block.mini <- rt_data %>% summarySE(measurevar = "rt",groupvars = c("subject","block"), na.rm = TRUE)
blockA.aov<- aov(rt_mean ~ block, block.mini)
summary(block.aov)
...
ggplot(check.sum) +
  geom_smooth(aes(target_num,rt_median, group = block)) +
  facet_wrap(. ~ block, nrow = 2)

check.sum2 <- rt_data %>% summarySE(.,measurevar = "rt",groupvars = c("target_num","block","tgt_pos"),na.rm = TRUE)
...
```{r}
check.sum.glm <- aov(rt ~ target_num*block, data = rt_data)
summary(check.sum.glm)
TukeyHSD(check.sum.glm, which = "block")

## check variance
#car::leveneTest(rt ~ target_num * block,rt_data) # variance across groups is homogenous
...
check.sum2.glm <- aov(rt ~ target_num*block*tgt_pos, data = rt_data)
summary(check.sum2.glm)

check.sum3.glm <- aov(rt ~ block*tgt_pos, data = rt_data)
summary(check.sum3.glm)


anova(check.sum.glm,check.sum2.glm,check.sum3.glm)
...
# summary(aov(detect_mean ~ tgt_pos, accuracy_data1)) # same thing
```
