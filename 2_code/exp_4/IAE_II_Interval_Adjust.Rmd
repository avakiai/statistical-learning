...
# 2. Summary Stats
```{r}
dial_points_sum1 <- dial_points_data %>%
...
# 3. Plot Start/End
## a. Control: Plot Dial Starts & Ends
```{r}
# Starts
ggplot(starting_points_data, aes(x = Dial, y = Curr_ISI, fill = Dial, colour = Dial)) +
  geom_flat_violin(position = position_nudge(x = .25, y = 0), adjust = 2) +
  geom_point(position = position_jitter(width = .15), size = .25) +
  geom_point(data = dial_start_sum2, aes(x = Dial, y = Curr_ISI_mean), 
             position = position_nudge(.25), colour = "BLACK") +
  geom_errorbar(data = dial_start_sum2, aes(x = Dial, y = Curr_ISI_mean, 
  ...
  ggsave('ia_starting_isi_rainclouds.png', width = w, height = h+1)

# Ends
ggplot(ending_points_data, aes(x = Dial, y = Curr_ISI, fill = Dial, colour = Dial)) +
  geom_flat_violin(position = position_nudge(x = .25, y = 0), adjust = 2) +
  geom_point(position = position_jitter(width = .15), size = .25) +
  geom_point(data = dial_end_sum2, aes(x = Dial, y = Curr_ISI_mean), 
             position = position_nudge(.25), colour = "BLACK") +
  geom_errorbar(data = dial_end_sum2, aes(x = Dial, y = Curr_ISI_mean, 
                                       ymin = Curr_ISI_mean-se, ymax = Curr_ISI_mean+se), 
                position = position_nudge(.25), colour = "BLACK", width = 0.1, size = 0.8) +
  # axes
  scale_x_discrete(name="Dials") + # scale x 
  ...
  theme(text = element_text(family = "LM Roman 10", face="bold")) + 
  ggtitle("Ending ISIs") + 
  ggsave('ia_ending_isi_rainclouds.png', width = w, height = h+1)
```

## b. Control: Plot Dial Facets
```{r}
# Starts
ggplot(starting_points_data, aes(x = Dial, y = Curr_ISI, fill = Dial, colour = Dial)) +
  geom_flat_violin(position = position_nudge(x = .25, y = 0), adjust = 2) +
  ...
  theme(text = element_text(family = "LM Roman 10", face="bold")) + 
  ggtitle("Starting ISIs") + 
  ggsave('ia_starting_isi_rainclouds_facets.png', width = w, height = h+1)
...
ggplot(ending_points_data, aes(x = Dial, y = Curr_ISI, fill = Dial, colour = Dial)) +
  ...
  theme(text = element_text(family = "LM Roman 10", face="bold")) + 
  ggtitle("Ending ISIs") + 
  ggsave('ia_ending_isi_rainclouds_facets.png', width = w, height = h+1)
```

## c. Test: Plot Position Starts & Ends
```{r}
# Starts
ggplot(starting_points_data, aes(x = Position, y = Curr_ISI, fill = Position, colour = Position)) +
  geom_flat_violin(position = position_nudge(x = .25, y = 0), adjust = 2) +
  geom_point(position = position_jitter(width = .15), size = .25) +
  geom_point(data = pos_start_sum2, aes(x = Position, y = Curr_ISI_mean), 
             position = position_nudge(.25), colour = "BLACK") +
  geom_errorbar(data = pos_start_sum2, aes(x = Position, y = Curr_ISI_mean, 
                                       ymin = Curr_ISI_mean-se, ymax = Curr_ISI_mean+se), 
                position = position_nudge(.25), colour = "BLACK", width = 0.1, size = 0.8) +
  # axes
  ...
  theme(text = element_text(family = "LM Roman 10", face="bold")) + 
  ggtitle("Starting ISIs") + 
  ggsave('ia_DV_starting_isi_rainclouds.png', width = w, height = h+1)

# Ends
ggplot(ending_points_data, aes(x = Position, y = Curr_ISI, fill = Position, colour = Position)) +
  geom_flat_violin(position = position_nudge(x = .25, y = 0), adjust = 2) +
  geom_point(position = position_jitter(width = .15), size = .25) +
  geom_point(data = pos_end_sum2, aes(x = Position, y = Curr_ISI_mean), 
             position = position_nudge(.25), colour = "BLACK") +
  geom_errorbar(data = pos_end_sum2, aes(x = Position, y = Curr_ISI_mean, 
                                       ymin = Curr_ISI_mean-se, ymax = Curr_ISI_mean+se), 
                position = position_nudge(.25), colour = "BLACK", width = 0.1, size = 0.8) +
  # axes
  scale_x_discrete(name="Gaps") + # scale x 
  ...
  theme(text = element_text(family = "LM Roman 10", face="bold")) + 
  ggtitle("Ending ISIs") + 
  ggsave('ia_DV_ending_isi_rainclouds.png', width = w, height = h+1)
...
ggplot(starting_points_data, aes(x = Position, y = Curr_ISI, fill = Position, colour = Position)) +
  ...
  theme(text = element_text(family = "LM Roman 10", face="bold")) + 
  ggtitle("Starting ISIs") + 
  ggsave('ia_DV_starting_isi_rainclouds_facets.png', width = w, height = h+1)
...
ggplot(ending_points_data, aes(x = Position, y = Curr_ISI, fill = Position, colour = Position)) +
  ...
  theme(text = element_text(family = "LM Roman 10", face="bold")) + 
  ggtitle("Ending ISIs") + 
  ggsave('ia_DV_ending_isi_rainclouds_facets.png', width = w, height = h+1)
```
  ...
  curr_dial_subj_blk <- curr_dial_subj[which(curr_dial_subj$Block==curr_blk),]
    ...
    curr_dial_subj_blk_trl <- curr_dial_subj_blk[which(curr_dial_subj_blk$Trial==curr_trl),]
      ...
      if (length(unique(curr_dial_subj_blk_trl_sess$Curr_ISI))==1 &
          (all(curr_dial_subj_blk_trl_sess$Curr_ISI == 600) | all(curr_dial_subj_blk_trl_sess$Curr_ISI == 0))) {
...
```{r}
hist(ending_points_data_filt$Curr_ISI)
```
...
```{r}
# - Loading Files
# raw_curr_data <- readLines(file.path(data_path, data_files[i]))
  # if (grepl("End_Time",tail(raw_curr_data,n=1))) {read_n_rows = length(skip_rows+1:(length(raw_curr_data)-1))
  # } else {read_n_rows = length(skip_rows+1:length(raw_curr_data))}
# - Detecting types
  # sapply(curr_data,typeof)
  # sapply(curr_data,class)
# - Troubleshooting a few files... 
  # momentarily...
  # [20:36, 4/16/2020] Peter Harrison: When you do as.numeric on a factor, it returns the underlying integer encoding, rather than the labels. This can be confusing when your original labels were also numbers. In these situations, you often want to do as.numeric(as.character(x)) instead.
   # if (subj_name %in% c("f0511","h0502", "m0207", "s2205")) {next}
    # sapply(curr_data,typeof)
  # sapply(curr_data,class)
  # unique(curr_data$Trial)
  # curr_data <- curr_data %>% 
  #   mutate(Subject = as.factor(Subject), 
  #          Block = as.numeric(Block),
  #          Trial = as.numeric(Trial),
