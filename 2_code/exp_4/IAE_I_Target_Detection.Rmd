...
raw_data <- read_csv(data_files[1])
...
hist(or_data2$rt)
ggqqplot(or_data2$rt,na.action=na.omit)
summary(or_data2$rt)
...
## Summarize
```{r}
accuracy_data1 <- data %>% summarySE(measurevar = "resp", groupvars = c("sess","tgt_pos","subject","target","tgt_word"),na.rm=TRUE) # collapse over trials

accuracy_data2 <- data %>% summarySE(measurevar = "resp", groupvars = c("sess","tgt_pos"),na.rm=TRUE) 

accuracy_data3 <- data %>% summarySE(measurevar = "resp", groupvars = c("sess","target","tgt_pos"),na.rm=TRUE) 

accuracy_data4 <- data[which(data$sess=="struct"),] %>% summarySE(measurevar = "resp", groupvars = c("tgt_word"),na.rm=TRUE)
```
...
ggplot() + 
 ...
 # geom_signif(comparisons=list(c("rand", "struct")), annotations="***", y_position=0.95, tip_length=0.3) + 
  ...
  ggsave('targetdetection_accuracy.png', width = w, height = h)

# ADAPT FROM EXPERIMENT 3 to 4!!!
# ggplot(accuracy_data2, aes(x = tgt_pos, y = detect_mean, fill = tgt_pos)) +
...
# dev.copy(pdf,file.path(fig_path,'exp3fig4_acc.pdf'), width = w, height = h)
# dev.off()
...
acc.table %>% 
    kable() %>%
     ...
     save_kable(file = 'targetdetection_accuracy_table.png', self_contained = T)  

em.acc <- em.sess.tgtpos$contrasts %>%
    summary(infer=TRUE) %>%
     rbind() %>%
     as.data.frame()
as.data.frame(em.acc) %>% 
    kable() %>%
     kable_styling(bootstrap_options = c("striped", "hover")) %>%
     save_kable(file = 'targetdetection_accuracy_table_contrasts.png', self_contained = T)  
...
### Stats
```{r}
target.lm <- lm(resp_mean ~ target*sess, data=accuracy_data1)
summary(target.lm)
Anova(target.lm)
# Contrasts
(em.target.sess <- emmeans(target.lm, specs = pairwise ~ target|sess))
plot(em.target.sess, comparisons = TRUE)
(em.sess.target <- emmeans(target.lm, specs = pairwise ~ sess|target))
plot(em.sess.target, comparisons = TRUE,)
...
```{r}
ggplot() + 
  geom_col(accuracy_data4, mapping=aes(tgt_word, resp_mean, group=as.factor(tgt_word), fill=as.factor(tgt_word)), 
           position = position_dodge(0.5), width=0.5) +
  geom_errorbar(accuracy_data4, mapping=aes(tgt_word, y=resp_mean, ymin=resp_mean-se, ymax=resp_mean+se,
           group=tgt_word), colour="BLACK",position=position_dodge(0.5), width = 0.3, size = 1) +
  ...
  theme_minimal() +
  theme(text = element_text(family = "LM Roman 10", face="bold")) +
  ggsave('targetdetection_accuracy_struct_word.png', width = w, height = h)
```

### Stats
```{r}
word.lm <- lm(resp_mean ~ tgt_word, data=dplyr::filter(accuracy_data1,sess=="struct"))
Anova(word.lm, type = '2')

# Contrasts...
em.word <- emmeans(word.lm, specs = pairwise ~ tgt_word)

  
```
There's no real difference between accuracy in detection syllables, as grouped by word. This analysis being only meaningful for struct condition, it at least tells us that all words features a sum of syllables that are roughly equally detectable. 
...
ggplot() +
  ...
  geom_point(data = data_SR_sum2, mapping = aes(x = tgt_pos, y = rt_median), colour = "BLACK") +
  geom_errorbar(data = data_SR_sum2, mapping = aes(x = tgt_pos, y = rt_median, ymin = rt_median-ci, ymax = rt_median+ci), colour = "BLACK", width = 0.1, size = 0.8) +
  geom_line(data = data_SR_sum2, mapping = aes(x = tgt_pos, y = rt_median, group = 1), colour = "BLACK", size = .9) +
  facet_grid(.~ sess) +
  scale_colour_brewer(palette = "Paired") +
  labs(colour= "Trial (Word)") + ylab('Median Response Time (ms) [bars = CI]') + xlab('Target Postion') +
  scale_x_discrete(limits=c(1:3)) +
  ...
  theme(text = element_text(family = "LM Roman 10", face="bold")) +
  ggtitle("a.") +
  ggsave('targetdetection_SR_median.png', width = w, height = h)
...
ggplot() +
  ...
  geom_point(data = data_RS_sum2, mapping = aes(x = tgt_pos, y = rt_median), colour = "BLACK") +
  geom_errorbar(data = data_RS_sum2, mapping = aes(x = tgt_pos, y = rt_median, ymin = rt_median-ci, ymax = rt_median+ci), colour = "BLACK", width = 0.1, size = 0.8) +
  geom_line(data = data_RS_sum2, mapping = aes(x = tgt_pos, y = rt_median, group = 1), colour = "BLACK", size = .9) +
  facet_grid(.~ sess) +
  scale_colour_brewer(palette = "Paired") +
  labs(colour= "Trial (Word)") + ylab('Median Response Time (ms) [bars = CI]') + xlab('Target Postion') +
  scale_x_discrete(limits=c(1:3)) +
  ...
  theme(text = element_text(family = "LM Roman 10", face="bold")) +
  ggtitle("b.") +
  ggsave('targetdetection_RS_median.png', width = w, height = h)
...
#  scale_y_continuous(limits=c(380,580)) +
  ...
  theme_classic() +
  theme(text = element_text(family = "LM Roman 10", face="bold")) +
  ggsave('exp4fig1b_rt_pos.png', width = w, height = h)
...
## b'. Session * Target Position
```{r}
# Now, let's knock out the factor we see having little effect and compare again. 
# glm.d.i <- glmer(rt_secs ~ sess + tgt_pos + (1 | cond_order/subject), data = data, family = Gamma)
#   summary(glm.d.i)
#   plot(glm.d.i)
#   qqnorm(resid(glm.d.i))
#   Anova(glm.d.i)
...
#anova(glm.d.i,glm.less.int) # Says go with i
#anova(glm.lesser.int,glm.d.i) # Says go with a
anova(glm.less.int,glm.full.int) # Says go with a
...
```{r}
(em.glm.d.tgt_pos.sess <- emmeans(glm.less.int, specs = pairwise ~ tgt_pos|sess, adjust = "tukey", transform = "response"))

em.glm.d.tgt_pos.sess.pairs <- em.glm.d.tgt_pos.sess$contrasts %>%
     summary(infer=TRUE) %>%
     rbind() %>%
     as.data.frame()

plot(em.glm.d.tgt_pos.sess, comparisons = TRUE)

pwpp(em.glm.d.tgt_pos.sess$emmeans, method = "pairwise")

emmip(glm.less.int, sess ~ tgt_pos) 

# --------------------- Uncorrected
(em.glm.d.tgt_pos.sess_unc <- emmeans(glm.less.int, specs = pairwise ~ tgt_pos|sess, adjust = "none", transform = "response"))
plot(em.glm.d.tgt_pos.sess_unc, comparisons = TRUE)

pwpp(em.glm.d.tgt_pos.sess_unc$emmeans, method = "pairwise")

```

#### session within position
```{r}
(em.glm.d.sess.tgt <- emmeans(glm.less.int, specs = pairwise ~ sess|tgt_pos, adjust = "tukey", transform = "response"))
...
plot(em.glm.d.sess.tgt, comparisons = TRUE)
pwpp(em.glm.d.sess.tgt$emmeans, method = "pairwise")

# --------------------- Uncorrected
(em.glm.d.sess.tgt.unc <- emmeans(glm.less.int, specs = pairwise ~ sess|tgt_pos, adjust = "none", transform = "response"))

```
LM 4/17: t-test on the diff between emm 1-2 between rand & struct. & repeat. This cannot be done out on the output of contrasts, but we would need to calculate mean differences for these pairs between subjects and run an analysis on those differences. See Chunk 2'a. Delta RT. 

#### each level of pos with each level of sess
```{r}
em.glm.d.tgt_pos.sess.b <- emmeans(glm.less.int, specs = pairwise ~ tgt_pos*sess, adjust = "tukey", transform = "response")

# Pair up positions in each session
some.pairs <- em.glm.d.tgt_pos.sess.b$contrasts[c(3,8,12)]

#plot(em.glm.d.tgt_pos.sess.b, comparisons = TRUE)
plot(some.pairs, comparisons = TRUE)

as.data.frame(some.pairs) %>% 
    kable() %>%
     kable_styling(bootstrap_options = c("striped", "hover")) %>%
     save_kable(file = 'targetdetection_contasts.png', self_contained = T)  

# --------------------- Uncorrected


em.glm.d.tgt_pos.sess.b.unc <- emmeans(glm.less.int, specs = pairwise ~ tgt_pos*sess, adjust = "none", transform = "response")

```

#### each level of pos/sess vs. rand:1
```{r}
# Control is Random: Position 1... 
(em.glm.d.tgt_pos.sess.ctrl <- emmeans(glm.less.int, specs = trt.vs.ctrl ~ tgt_pos:sess, adjust = "tukey", transform = "response",reverse = TRUE))

plot(em.glm.d.tgt_pos.sess.ctrl,comparisons=TRUE)

# Confidence Intervals
em.glm.d.tgt_pos.sess.ctrl$contrasts %>%
    summary(infer=TRUE)


#------------------------------- Uncorrected
(em.glm.d.tgt_pos.sess.ctrl.unc <- emmeans(glm.less.int, specs = trt.vs.ctrl ~ tgt_pos:sess, adjust = "none", transform = "response",reverse = TRUE))
```

## c. Session * Condition Order * Target Position
```{r}
# glm.c.i <- glmer(rt_secs ~ sess + cond_order + tgt_pos + (1 | cond_order/subject), data = data, family = Gamma)
#   summary(glm.c.i)  
#   plot(glm.c.i)
#   qqnorm(resid(glm.c.i))
#   Anova(glm.c.i)
  # Note!: an effect of session and of target position, but not condition order

# anova(glm.lesser.int,glm.c.i)
# Aha! Now we see a significant difference in the fit of these models. glm.c.i appears to fit less well, oddly...
  
glm.fuller.int <- glmer(rt_secs ~ sess * cond_order * tgt_pos + (1 | cond_order/subject), data = data, family = Gamma)
  summary(glm.fuller.int) 
  plot(resid(glm.fuller.int))
  qqnorm(resid(glm.fuller.int))
  Anova(glm.fuller.int)
  # Note: session, tgt position, as well as interaction sess:pos, and a marginal order:pos

anova(glm.less.int,glm.fuller.int)
  # Go with model glm.c.i it seems...
```
* NB: Plot interactions: sess* tgt_pos, cond_order* tgt_pos & three-way-interaction. 

## c': Session * Condition Order * Position x Random Slope
```{r}

glm.fuller.slope <- glmer(rt_secs ~ sess * cond_order * tgt_pos + (tgt_pos | cond_order/subject), data = data, family = Gamma)
  summary(glm.fuller.slope) 
  plot(resid(glm.fuller.slope))
  qqnorm(resid(glm.fuller.slope))
  Anova(glm.fuller.slope)
