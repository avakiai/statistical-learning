...
# too comp to explain... 
setwd("C:/Users/Ava/Documents/R/win-library/4.0/ggpattern-master/R")
ggpattern.sources = list.files()
sapply(ggpattern.sources, source)
...
#        strip.background = element_rect(linetype = c("dashed"))) +
 # ggtitle("b.") +
...
```{r}
# library(rstatix)
# Friedman's test for non-parametric two-way dependent samples
# # turning into factors, as suggested, and keeping only complete cases returns error that this is not an unreplicated complete block design
# fried.1 <- friedman.test(rt ~ target_num | subject, (data)[!is.na(data$rt),] %>% mutate(trial = as.factor(trial),target_num = as.factor(target_num)))
# # # as a matrix, or as db with complete cases doesn't help; says object not a matrix or not unrep comp blk design
# 
# fried.1 <- friedman.test(rt_median ~ target_num | trial, as.matrix(check.4[complete.cases(check.4),] %>% mutate(trial = as.factor(trial),target_num = as.factor(target_num))))
...
## check variance
# check normality | normal enough, 0.048
# shapiro.test(sample(residuals(check.sum.glm),5000)) 
# plot(check.sum.glm,1)
# plot(check.sum.glm,2)
# descdist(rt_data$rt[!is.na(rt_data$rt)])
# gofstat(list(fitdist(rt_data$rt[!is.na(rt_data$rt)],"norm"),
#              #fitdist(log(rt_data$rt[!is.na(rt_data$rt)]),"lnorm"),
#              fitdist(rt_data$rt[!is.na(rt_data$rt)],"gamma")))


# check.lm.4.2 <- lm(rt ~ target_num*trial*sess, data = data)
# summary(check.lm.4.2)
# plot(resid(check.lm.4.2))
# Anova(check.lm.4.2)
# 
# # Three way between target num, sess and pos
# check.lm.4.3 <- lm(rt ~ target_num*trial*sess*tgt_pos, data = data)
# summary(check.lm.4.3)
# plot(resid(check.lm.4.3))
# Anova(check.lm.4.3)
# 
# # ... so drop trial 
# check.lm.4.4 <- lm(rt ~ target_num*sess*tgt_pos, data = data)
# summary(check.lm.4.3)
# plot(resid(check.lm.4.3))
# Anova(check.lm.4.4)
# 
# anova(check.lm.4.4,check.lm.4.3)
# # Without trial fits better
# 
# 
# (emmeans(check.lm.4.4, specs = pairwise ~ target_num|sess, adjust = "Tukey", transform = "response"))
```
