...
# Load Saved TD
```{r}
data <- read_csv("C:/Users/Ava/Desktop/Experiments/interval_adjust/r_scripts_v1/exp_4_rt_data.csv") %>%
...
em.tgt_pos.sess$contrasts %>%
     #summary(infer=TRUE) %>%
     rbind() %>%
     as.data.frame() -> pos.sess.c
pos.sess.c_out <- pos.sess.c %>%
  mutate(session = sess,
        "estimated marginal diff." = round(estimate,digits = 3),
  ...
  dplyr::select(8,1,9,3:4,10:12)

stargazer(pos.sess.c_out, 
          type = "html", 
          out = "4_pos_sess_con.doc", summary = FALSE, rownames = FALSE)
...
png(file = 'exp4figS1.png', width = w, height = h, res = 300, units = "in")
emmip(glm.less.slope3, tgt_pos ~ sess, xlab = "session", CIs = TRUE) +
  labs(color = "position") + 
  scale_color_brewer(palette="Dark2") + 
