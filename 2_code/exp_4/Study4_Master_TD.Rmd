...
```{r}
check.sum.aov <- aov(rt ~ target_num*trial, data = data)
summary(check.sum.aov)
#TukeyHSD(check.sum.aov, which = "target_num:trial")

#emmeans(check.sum.aov, specs = pairwise ~ target_num|trial)
...
check.sum2.aov <- aov(rt ~ target_num*trial*tgt_pos, data = rt_data)
summary(check.sum2.aov)

check.sum3.aov <- aov(rt ~ trial*tgt_pos, data = rt_data)
summary(check.sum3.aov)


#anova(check.sum.aov,check.sum2.aov)




check.4 <- data %>% summarySE(.,measurevar = "rt",groupvars = c("target_num","trial"),na.rm = TRUE)

ggplot(check.4) +
  geom_smooth(aes(target_num,rt_median, group = trial)) +
  facet_wrap(. ~ trial, nrow = 2)

check.4.2 <- data %>% summarySE(.,measurevar = "rt",groupvars = c("target_num","trial","tgt_pos"),na.rm = TRUE)
