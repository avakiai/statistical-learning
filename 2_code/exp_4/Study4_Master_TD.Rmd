...
## Summarize
```{r}
data %>% summarySE(measurevar = "resp",na.rm=TRUE) base_acc1 <- 
...
accuracy_data6 <- data %>% summarySE(measurevar = "resp", groupvars = c("sess","target"),na.rm=TRUE) %>%
  mutate(target = factor(target, levels = c("nu","ga","di","ro","ki","se","mi","po","la","za","be","tu")))
...
ggplot(accuracy_data5, aes(x = target, y = resp_mean)) +
  ...
  geom_flat_violin(position = position_nudge(x = 0.2, y = 0), adjust = 1, trim = FALSE,
                   fill = "gray") + 
  geom_point(accuracy_data6, mapping = aes(target, resp_mean), position = position_nudge(x = 0.2, y = 0)) +
  geom_errorbar(accuracy_data6, mapping=aes(target, y=resp_mean, ymin=resp_mean-se,
                                            ymax=resp_mean+se),
                position = position_nudge(x = 0.2, y = 0), width = 0.05, size = 1) +
  facet_grid(.~sess) + 
  scale_color_brewer(palette="Dark2") + 
  ...
  theme_minimal() +
  theme(text = element_text(family = "LM Roman 10", face="bold")) +
  ggsave(file.path(fig_path,'exp4_acc_fig3.png'),width=11,height=6)
```
