...
# mean(within.OP.samp, na.rm=TRUE)
# length(within.OP.samp)
# within.OP.samp <- double()
# within.OP.means <- double()
# for (boot in 1:200) {
#   aktuel <- as.numeric(replicate(1,sample(within.OP, size = n.op, replace = TRUE)))
#   aktuel.mittel <- mean(aktuel)
#   within.OP.samp <- append(within.OP.samp,aktuel)
#   within.OP.means <- append(within.OP.means,aktuel.mittel)
# }  
...
# mean(across.OP.samp, na.rm = TRUE)
#length(across.OP.samp)
# across.OP.samp <- double()
# across.OP.means <- double()
# for (boot in 1:200) {
#   aktuel <- as.numeric(replicate(1,sample(across.OP, size = n.op, replace = TRUE)))
#   aktuel.mittel <- mean(aktuel)
#   across.OP.samp <- append(across.OP.samp,aktuel)
#   across.OP.means <- append(across.OP.means,aktuel.mittel)
# } 
...
# mean(within.TP.samp,na.rm = TRUE)
...
# mean(across.TP.samp)
# within.TP.samp <- double()
# within.TP.means <- double()
# for (boot in 1:200) {
#   aktuel <- as.numeric(replicate(1,sample(within.TP, size = n.op, replace = TRUE)))
#   aktuel.mittel <- mean(aktuel)
#   within.TP.samp <- append(within.TP.samp,aktuel)
#   within.TP.means <- append(within.TP.means,aktuel.mittel)
# } 
# across.TP.samp <- double()
# across.TP.means <- double()
# for (boot in 1:200) {
#   aktuel <- as.numeric(replicate(1,sample(across.TP, size = n.op, replace = TRUE)))
#   aktuel.mittel <- mean(aktuel)
#   across.TP.samp <- append(across.TP.samp,aktuel)
#   across.TP.means <- append(across.TP.means,aktuel.mittel)
# } 
...
# mean(within.WI.samp,na.rm=TRUE)
...
# mean(across.WI.samp, na.rm=TRUE)
# within.WI.samp <- double()
# within.WI.means <- double()
# for (boot in 1:200) {
#   aktuel <- as.numeric(replicate(1,sample(within.WI, size = n.op, replace = TRUE)))
#   aktuel.mittel <- mean(aktuel)
#   within.WI.samp <- append(within.WI.samp,aktuel)
#   within.WI.means <- append(within.WI.means,aktuel.mittel)
# } 
# across.WI.samp <- double()
# across.WI.means <- double()
# for (boot in 1:200) {
#   aktuel <- as.numeric(replicate(1,sample(across.WI, size = n.op, replace = TRUE)))
#   aktuel.mittel <- mean(aktuel)
#   across.WI.samp <- append(across.WI.samp,aktuel)
#   across.WI.means <- append(across.WI.means,aktuel.mittel)
# } 
...
# mean(within.di.samp, na.rm = TRUE)
...
# mean(across.di.samp, na.rm =TRUE)
# within.di.samp <- double()
# within.di.means <- double()
# for (boot in 1:200) {
#   aktuel <- as.numeric(replicate(1,sample(within.di, size = n.op, replace = TRUE)))
#   aktuel.mittel <- mean(aktuel)
#   within.di.samp <- append(within.di.samp,aktuel)
#   within.di.means <- append(within.di.means,aktuel.mittel)
# } 
# across.di.samp <- double()
# across.di.means <- double()
# for (boot in 1:200) {
#   aktuel <- as.numeric(replicate(1,sample(across.di, size = n.op, replace = TRUE)))
#   aktuel.mittel <- mean(aktuel)
#   across.di.samp <- append(across.di.samp,aktuel)
#   across.di.means <- append(across.di.means,aktuel.mittel)
# } 
...
ggplot(WA.wilcox) + 
  ...
  ggsave(file.path(fig_path, paste0('/fig9_',name,'_run_x_stars_col.png')), width = 7, height = 5)  
...
# Bootstrap/permute
#within.OP.samp <- sample(within.OP, size = n.op, replace = FALSE) 
within.OP.mean <- mean(within.OP) 
#across.OP.samp <- sample(across.OP, size = n.op, replace = FALSE) 
...
#within.WI.samp <- sample(within.WI, size = n.wi, replace = FALSE)
within.WI.mean <- mean(within.WI)
#across.WI.samp <- sample(across.WI, size = n.wi, replace = FALSE)
...
```{r}
# Scaling within - across means doesn't really solve the outlier issue, but perhaps we can just remove those anyway. 
# If we scale, it should be per participant. 

# Look, these are means of Fisher transforms, so means of zscores. I think we can safely substract them without much fear. 
# plot(corr.table$mean.with-corr.table$mean.accr)
# plot(log(corr.table$mean.with-corr.table$mean.accr))
# 
# hist((corr.table$mean.with)-(corr.table$mean.accr),breaks = 30)
# hist(scale(corr.table$mean.with-corr.table$mean.accr), breaks = 30) # z 
# hist(scale(corr.table$mean.with-corr.table$mean.accr, center = 0),breaks = 30)


# ... but the log causes quite a bit of data loss. Not sure why. 

# I considered also using a normalization method such as x1 - x2 / min(x1,x2)-max(x1,x2), but the min/max normalization seems appropriate when you have one sample, not two. Also, it's highly sensitive to outliers, obviously. 

# There remains the z-normalization. 

# Ratio is odd cause it causes errors if mean of across group is zero. 
```
...
# rho = c(round(corr.ordp$estimate, digits = 2),
#   round(corr.prob$estimate, digits = 2),
#   round(corr.word$estimate, digits = 2),
#   round(corr.dups$estimate, digits = 2)),
# p = c(round(corr.ordp$p.value, digits = 2),
#   round(corr.prob$p.value, digits = 2),
#   round(corr.word$p.value, digits = 2),
#   round(corr.dups$p.value, digits = 2))
```
...
#---------------------------------------- if facets, plot on each plot the stats
  geom_text(data = plots.text, mapping = aes(x = -Inf, y = -Inf, label = label), color = "black", hjust = -0.1, vjust = -1) +
  facet_wrap(. ~ test,nrow = 2)+ 
  geom_vline(xintercept = 0, color = "gray", linetype="dashed") + 
  theme_classic() +
  theme(text = element_text(family = "LM Roman 10", face="bold",size=14), 
        legend.title = element_text(""),  legend.position = "top") +
  ggsave(file.path(fig_path,'fig10_corr_sub_final_labels_x_facet2_black.png'),width = 8, height = 7)  
